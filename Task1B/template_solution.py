...
def fit_logistic_regression(X, y):
    ...
    for _ in range(max_iter):
        ...
        # gradient of negative log-likelihood + L2 regularization
        grad = X_transformed.T @ (p - y) + reg * weights

        # Hessian
        r = p * (1.0 - p)                       # shape (n,)
        XR = X_transformed * r[:, None]         # each row scaled by r_i
        H = X_transformed.T @ XR + reg * np.eye(d)
